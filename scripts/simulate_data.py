...
def simulate_data(speech_dir, noise_dir, rir_dir, csv, out_dir, speech_ext, noise_ext, rir_ext, fs=16000, out_dtype='int16', save_mix=True, save_separate=True):
    ...
    for _, row in simu_definition.iterrows():
        ...
        n_chans = 1 if speech_rir_sig.ndim == 1 else speech_rir_sig.shape[0]
        ...
        for idx, (rev_speech, rev_noise) in enumerate(zip(reverb_speech_sig, reverb_noise_sig)):
            ...
            corrupt_path = row['corrupted'] if 'corrupted' in row else row['speech']
            if save_separate:
                out_speech_path = os.path.join(out_dir, corrupt_path + (f'_{idx}_speech.wav' if n_chans > 1 else '_speech.wav'))
                out_noise_path  = os.path.join(out_dir, corrupt_path + (f'_{idx}_noise.wav'  if n_chans > 1 else '_noise.wav'))
            ...
            if save_mix:
                out_mix_path = os.path.join(out_dir, 'mix', corrupt_path + (f'_{idx}.wav' if n_chans > 1 else '.wav'))
